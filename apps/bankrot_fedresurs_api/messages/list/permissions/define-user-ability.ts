...
type MessagesListAbility = PureAbility<[string, typeof crudEntityName]>;
const messagesListAbility = PureAbility as AbilityClass<MessagesListAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForMessagesList({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<MessagesListAbility>(messagesListAbility);
