...
type MessPublisherListAbility = PureAbility<[string, typeof crudEntityName]>;
const messPublisherListAbility =
  PureAbility as AbilityClass<MessPublisherListAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForMessagesPublisherList({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<MessPublisherListAbility>(
    messPublisherListAbility
  );
