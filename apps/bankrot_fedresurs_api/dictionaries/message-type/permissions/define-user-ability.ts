...
type DictMessageTypeAbility = PureAbility<[string, typeof crudEntityName]>;
const dictMessageTypeAbility =
  PureAbility as AbilityClass<DictMessageTypeAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForDictMessageType({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<DictMessageTypeAbility>(
    dictMessageTypeAbility
  );
