...
type DictAuctPlatformsAbility = PureAbility<[string, typeof crudEntityName]>;
const dictAuctPlatformsAbility =
  PureAbility as AbilityClass<DictAuctPlatformsAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForDictAuctPlatforms({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<DictAuctPlatformsAbility>(
    dictAuctPlatformsAbility
  );
