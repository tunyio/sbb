...
type DictAuctPlatformsSroAbility = PureAbility<[string, typeof crudEntityName]>;
const dictAuctPlatformsSroAbility =
  PureAbility as AbilityClass<DictAuctPlatformsSroAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForDictAuctPlatformsSro({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<DictAuctPlatformsSroAbility>(
    dictAuctPlatformsSroAbility
  );
