...
type DictSroAbility = PureAbility<[string, typeof crudEntityName]>;
const dictSroAbility = PureAbility as AbilityClass<DictSroAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForDictSro({ user, method }: ProceedCrudAuth) {
  const builder = new AbilityBuilder<DictSroAbility>(dictSroAbility);
