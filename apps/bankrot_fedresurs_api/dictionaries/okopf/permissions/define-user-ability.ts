...
type DictOkopfAbility = PureAbility<[string, typeof crudEntityName]>;
const dictOkopfAbility = PureAbility as AbilityClass<DictOkopfAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForDictOkopf({ user, method }: ProceedCrudAuth) {
  const builder = new AbilityBuilder<DictOkopfAbility>(dictOkopfAbility);
