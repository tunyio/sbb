...
type DictArbCourtAbility = PureAbility<[string, typeof crudEntityName]>;
const dictArbCourtAbility = PureAbility as AbilityClass<DictArbCourtAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForDictArbitrationCourt({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<DictArbCourtAbility>(dictArbCourtAbility);
