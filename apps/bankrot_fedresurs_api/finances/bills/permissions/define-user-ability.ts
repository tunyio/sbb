...
type FinancesBillsAbility = PureAbility<[string, typeof crudEntityName]>;
const financesBillsAbility = PureAbility as AbilityClass<FinancesBillsAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForFinancesBills({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<FinancesBillsAbility>(financesBillsAbility);
