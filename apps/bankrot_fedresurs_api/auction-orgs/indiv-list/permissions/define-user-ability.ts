...
type AuctOrgsIndivListAbility = PureAbility<[string, typeof crudEntityName]>;
const auctOrgsIndivListAbility =
  PureAbility as AbilityClass<AuctOrgsIndivListAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForAuctionOrgsIndivList({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<AuctOrgsIndivListAbility>(
    auctOrgsIndivListAbility
  );
