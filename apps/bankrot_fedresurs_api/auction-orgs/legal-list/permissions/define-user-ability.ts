...
type AuctOrgsLegalListAbility = PureAbility<[string, typeof crudEntityName]>;
const auctOrgsLegalListAbility =
  PureAbility as AbilityClass<AuctOrgsLegalListAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForAuctionOrgsLegalList({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<AuctOrgsLegalListAbility>(
    auctOrgsLegalListAbility
  );
