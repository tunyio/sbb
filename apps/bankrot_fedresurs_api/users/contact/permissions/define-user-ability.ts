...
type UsersContactsAbility = PureAbility<[string, typeof crudEntityName]>;
const usersContactsAbility = PureAbility as AbilityClass<UsersContactsAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForUsersContacts({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<UsersContactsAbility>(usersContactsAbility);
