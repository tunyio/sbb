...
type ArbManagersActionsAbility = PureAbility<[string, typeof crudEntityName]>;
const arbManagersActionsAbility =
  PureAbility as AbilityClass<ArbManagersActionsAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForArbManagersActions({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<ArbManagersActionsAbility>(
    arbManagersActionsAbility
  );
