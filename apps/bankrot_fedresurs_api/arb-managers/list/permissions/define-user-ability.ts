...
type ArbManagersListAbility = PureAbility<[string, typeof crudEntityName]>;
const arbManagersListAbility =
  PureAbility as AbilityClass<ArbManagersListAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForArbManagersList({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<ArbManagersListAbility>(
    arbManagersListAbility
  );
