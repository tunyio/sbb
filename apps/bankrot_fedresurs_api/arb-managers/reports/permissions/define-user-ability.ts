...
type ArbManagersReportsAbility = PureAbility<[string, typeof crudEntityName]>;
const arbManagersReportsAbility =
  PureAbility as AbilityClass<ArbManagersReportsAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForArbManagersReports({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<ArbManagersReportsAbility>(
    arbManagersReportsAbility
  );
