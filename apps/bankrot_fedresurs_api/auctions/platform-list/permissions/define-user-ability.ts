...
type AuctionsPlatformListAbility = PureAbility<[string, typeof crudEntityName]>;
const auctionsPlatformListAbility =
  PureAbility as AbilityClass<AuctionsPlatformListAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForAuctionsPlatformList({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<AuctionsPlatformListAbility>(
    auctionsPlatformListAbility
  );
