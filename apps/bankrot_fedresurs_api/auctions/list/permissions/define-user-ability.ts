...
type AuctionsListAbility = PureAbility<[string, typeof crudEntityName]>;
const auctionsListAbility = PureAbility as AbilityClass<AuctionsListAbility>;
const fieldMatcher: FieldMatcher = (fields) => (field) => fields.includes(field);

export function defineCrudAbilityForAuctionsList({
  user,
  method
}: ProceedCrudAuth) {
  const builder = new AbilityBuilder<AuctionsListAbility>(auctionsListAbility);
